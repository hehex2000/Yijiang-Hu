...
def load_panels(codes, start, end, warmup_days=400):
    ...
    return open_r, high_r, low_r, close_r, pre_close_r, adj_d
...
def build_signals(all_dates, open_r, high_r, low_r, close_r, pre_close_r, adj_d, industry_map, cfg):
    ...
    return dict(open_raw=open_raw, close_raw=close_raw, pre_close_raw=pre_close_raw, close_h=close_h,
                ...
                ma_below=ma_below, below_key=below_key, key_level=key_level)
...
def run_window(start, end, cfg, layers):
    ...
    open_r, high_r, low_r, close_r, pre_close_r, adj_d = load_panels(all_codes, start, end)
    industry_map = load_industry()
    sig = build_signals(all_dates, open_r, high_r, low_r, close_r, pre_close_r, adj_d, industry_map, cfg)

    # 市场门控: 沪深300收盘价>50日均线，且近10日累计收益率≥0
    idx_close = load_index(INDEX_MARKET, start, end)
    mkt = idx_close.reindex(all_dates)
    mkt_ma = mkt.rolling(50, min_periods=50).mean()
    mkt_10d_ret = mkt / mkt.shift(10) - 1.0
    bull = (mkt > mkt_ma) & (mkt_10d_ret >= 0) & mkt_ma.notna() & mkt_10d_ret.notna()
    ...
    cash = CAPITAL
    positions = {}      # code -> dict(shares, entry_open, entry_hfq, key_hfq, high_water_mark)
    ...
    for i in range(n):
        ...
        bull_t = bool(bull.iloc[i]) if i < len(bull) else False
        ...
        # 市场转熊 → 整批关信号（若启用）
        force_exit_all = (market_exit and use_market and not bull_t)
        ma_i = ma_sets[i]
        for code, pos in list(positions.items()):
            if code not in univ:
                sell_exec.append((code, open_raw.iloc[i].get(code))); continue
            ch = close_h_arr[i, code2idx[code]] if code in code2idx else np.nan
            if pd.isna(ch):
                continue
            # 更新持仓最高价
            pos["high_water_mark"] = max(pos["high_water_mark"], ch)
            exit_now = False
            exit_price = None
            if use_exit:
                # 跟踪止盈: 从最高价回撤8%
                if trailing_stop > 0 and pos["high_water_mark"] > 0 and ch < pos["high_water_mark"] * (1 - trailing_stop):
                    exit_now = True
                # 硬止损: 跌破成本价5%
                elif stop_loss > 0 and not pd.isna(pos["entry_hfq"]) and ch < pos["entry_hfq"] * (1 - stop_loss):
                    exit_now = True
                # 跌破突破位下方3% → 失效离场
                elif not pd.isna(pos["key_hfq"]) and ch < pos["key_hfq"] * (1 - exit_pct):
                    exit_now = True
                # 跌破MA
                elif code in ma_i:
                    exit_now = True
        ...
        # --- 3) 新开仓（T-1 信号, T 开盘执行）---
        can_open = (not use_market) or bull_t
        prev_i = i - 1
...
def main():
    ...
    ap.add_argument("--trailing-stop", type=float, default=0.08, help="跟踪止盈回撤比例")

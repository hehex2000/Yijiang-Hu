...
# -*- coding: utf-8 -*-
"""
简化回测脚本 - 运行MACD/RSI、双均线、海龟策略并生成比对表
"""
...
from backtest.smart_switch_strategy import SmartSwitchStrategy
...
def run_backtest_for_stock(stock_code: str, start_date: str, end_date: str):
    """
    对单只股票运行5种策略，返回结果
    策略：买入持有、月定投(无TP/SL)、双均线、MACD/KDJ、MACD/RSI
    """
    print(f"\n{'='*60}")
    ...
    smart_orders = smart.run(df, stock_code=stock_code)
    df_daily_smart = pd.DataFrame(smart.daily_values)
    smart_metrics = calculate_metrics(smart_orders, df_daily_smart, 200000)
    results['智能切换(MA200)'] = {
        'orders': smart_orders,
        'total_return': smart_metrics.get('total_return', 0),
        'win_rate': smart_metrics.get('win_rate', 0),
        'trade_count': len(smart_orders)
    }
    print(f"    总收益率: {smart_metrics.get('total_return', 0)*100:.2f}%")
    print(f"    胜率: {smart_metrics.get('win_rate', 0)*100:.2f}%")
    print(f"    交易次数: {len(smart_orders)}\n")
...
def main():
    """主函数"""
    print("="*60)
    print("多策略回测 - 买入持有 vs 月定投 vs 双均线 vs MACD/KDJ vs MACD/RSI")
    print("="*60)
    ...
    # 生成比对表
    print(f"\n\n{'='*80}")
    print("回测结果比对表（2021年全年）")
    print(f"{'='*80}\n")
    
    # 表头
    header = f"{'股票':<8} {'策略':<20} {'总收益率':<14} {'胜率':<12} {'交易次数':<10}"
    print(header)
    print("-"*80)
    
    # 表格内容
    strategy_order = ['买入持有', '月定投(无TP/SL)', '双均线(MA5/20)', 'MACD/KDJ', 'MACD/RSI', '智能切换(MA200)']
    for stock_code in stocks:
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        for stock_code in stocks:
            if stock_code in all_results:
                ...
                results = all_results[stock_code]
                for strategy_name in ['买入持有', '月定投(无TP/SL)', '双均线(MA5/20)', 'MACD/KDJ', 'MACD/RSI', '智能切换(MA200)']:
                    if strategy_name in results:
                        r = results[strategy_name]
                        if strategy_name == '买入持有':
                            win_str = '-'

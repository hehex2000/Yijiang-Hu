"""
diag_macd_jim.py —— MACD 状态机增强版轻量冒烟测试（合成数据，无需 DB）

验证：
  1) 基础版(MacdTimingPlugin) 与 状态机版(MacdJimPlugin) 都能跑通不崩溃
  2) 状态机版交易数 <= 基础版（信号漏斗：震荡/弱金叉被方向过滤挡掉）
  3) 回报可计算、daily_values 正常

用法（Windows PowerShell）：
  cd C:/Users/99395/WorkBuddy/multi_factor_selection
  ./venv_ml/Scripts/python.exe diag_macd_jim.py
"""
import numpy as np
import pandas as pd
...
def main():
    df = make_df()
    # 复制 macd_timing 配置（与 config.py 一致；state_machine 由各自类决定）
    base_cfg = {
        "fast": 12, "slow": 26, "signal": 9, "zero_line": False,
    ...
    r_base = run_one(MacdTimingPlugin, base_cfg, df.copy())
    r_jim = run_one(MacdJimPlugin, jim_cfg, df.copy())

    n_base = len(r_base["trades"])
    n_jim = len(r_jim["trades"])
    print(f"[基础版 macd_timing] 收益={r_base['returns']:+.2f}%  交易={n_base}笔  daily={len(r_base['daily_values'])}")
    print(f"[状态机 macd_jim]    收益={r_jim['returns']:+.2f}%  交易={n_jim}笔  daily={len(r_jim['daily_values'])}")

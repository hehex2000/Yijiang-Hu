...
START = os.environ.get("VULN_START", "20180101")
END = os.environ.get("VULN_END", "20251231")
ENABLE_STOP = os.environ.get("VULN_STOP", "0") == "1"
ENABLE_STYLE = os.environ.get("VULN_STYLE", "0") == "1"
MODE_TAG = ("_def" if (ENABLE_STOP or ENABLE_STYLE) else "") + \
           ("_sl" if ENABLE_STOP else "") + ("_sw" if ENABLE_STYLE else "")
START_SUFFIX = f"_{START}" if START != "20180101" else ""
NAV_CACHE = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         f"data/results/sc_vuln_nav_{START}_{END}{MODE_TAG}{START_SUFFIX}.json")
...
def load_nav():
    ...
    res = run_backtest(start_date=START, end_date=END, hold_count=7,
                       ...
                       enable_stop_loss=ENABLE_STOP, style_switch=ENABLE_STYLE)
...
def main():
    nav = load_nav()
    mode = "防御版(stop_loss+style_switch开启)" if (ENABLE_STOP or ENABLE_STYLE) else "基线(默认参数·两道防御关)"
    print(f"\n[模式] {mode}  | 区间 {START}~{END}  | stop_loss={ENABLE_STOP} style_switch={ENABLE_STYLE}")
    ...
    out_csv = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                           f"data/results/sc_vuln_matrix_{START}_{END}{MODE_TAG}{START_SUFFIX}.csv")
    with open(out_csv, "w", newline="", encoding="utf-8-sig") as f:
        w = _csv.writer(f)
        w.writerow(["小市值轮动策略 · 脆弱性体检 (生态位矩阵)", START, END])
        w.writerow([])
        w.writerow(["流动性 \\ 小盘占优度", "低流动性", "中流动性", "高流动性"])
        for si in range(3):
            row = [SNAME[si]]
            for li in range(3):
                cell = matrix[(si, li)]
                row.append(f"{cell[0]*100:+.1f}%" if cell else "n/a")
            w.writerow(row)
        w.writerow([])
        w.writerow(["已知窗口", "策略区间收益", "基准(中证2000)区间收益", "超额"])
        for name, a, b in windows:
            seg = [(d, v) for d, v in zip(dates, vals) if int(a) <= d <= int(b)]
            bseg = [(d, v) for d, v in zip(dates, bench) if int(a) <= d <= int(b)]
            if len(seg) < 2 or len(bseg) < 2:
                w.writerow([name, "数据不足", "", ""]); continue
            s_ret = seg[-1][1] / seg[0][1] - 1
            bl, bh = bseg[0][1], bseg[-1][1]
            b_ret = (bh / bl - 1) if bl else 0
            w.writerow([name, f"{s_ret*100:+.1f}%", f"{b_ret*100:+.1f}%", f"{(s_ret-b_ret)*100:+.1f}%"])
        w.writerow([])
        w.writerow(["全样本基线", f"{full_s*100:+.1f}%", f"{full_b*100:+.1f}%", f"{(full_s-full_b)*100:+.1f}%"])
    print(f"\n[OK] 矩阵已导出 CSV → {out_csv}")

    ...
    for i in range(n):
        ...
        for code, pos in list(positions.items()):
            ...
            if use_exit:
                ...
                # 关键修正：卖出决策在day i开盘时做出，只能用day i-1及之前的数据
                # 所以检查entry_idx+1和entry_idx+2的high时，需要确保entry_idx+2 <= i-1
                # 即 i >= entry_idx + 3（延迟一天判断，避免同日未来函数）
...
def main():
    ...
    if args.ablation:
        ...
        print(f"改进: sector_mode={args.sector_mode} vol_size={args.vol_size} vol_skip={args.vol_skip} vol_lookback={args.vol_lookback}")

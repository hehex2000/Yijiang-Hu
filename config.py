...
# 三、基准口径配置（全收益切换，详见 bench_index.py）
# ============================================================

# 基准口径。推荐 "auto"：按策略 NAV 的计价口径自动对齐（两端同含或同不含分红），
#   这是唯一能保证"超额收益"不被系统性高估/低估的设置。
#   auto         = NAV 用 raw(不含分红) → 基准取价格指数；NAV 用 hfq(含分红) → 基准取全收益
#   tr_official  = 强制官方全收益（NAV 必须是 hfq，否则超额被低估）
#   total_return = 强制自建全收益
#   price        = 强制价格指数（NAV 必须是 raw，否则超额被高估）
#   net          = 强制官方净收益（已扣红利税）
# ⚠️ 用非 auto 值前请确认策略 NAV 口径与之匹配；不匹配时报告会打印口径错配告警。
BENCHMARK_MODE = "auto"
...
STRATEGIES = {
    ...
    "mean_reversion": {
        ...
        # ── 组合级共享资金池（★2026-09-07 起默认开启，用户已拍板）────────────
        #  背景：平台主流程是「逐票独立 + 资金 1/N 均分」，单票仓位被锁死在
        #        position_pct/N（40 只 × 0.50 = 每票 1.25%），组合总暴露仅 ~8.5%，
        #        91.5% 资金闲置，CAGR 被压到 3.40%（组合级共享池 f=10% 为 13.96%）。
        #  开启后：不再逐票独立回测，改用 backtest/portfolio_engine.py ——
        #        一个资金池、所有信号按同一时间轴抢钱、逐日盯市、单笔 = 权益 × f。
        #  ⚠️ 开启会改变该策略的历史回测数字（口径不同，不可与逐票结果混用）。
        #  ⚠️ 生效范围：**仅插件类策略**（skey in plugins）。旧函数式策略
        #     （strategy_funcs）不走这个分支，开关对它们无效，仍逐票跑。
        "portfolio_shared_pool": True,
        #  单笔投入占【当前权益】的比例。
        #  None ⇒ **自动模式** f = clamp(2/N, 0.05, 0.25)，N = 实际股票只数。
        #    实证（run_small_capital_scan.py，zz800 / 20200103~20260825）：
        #      N=5 →0.25   N=10 →0.20   N=20 →0.10   N=40 →0.05   ⇒ f×N ≈ 2.0 恒定
        #    即「池子越小 f 越大」。★ 40 只池得出的 f=0.10 直接套到 5 只池是错的：
        #      N=5 时 f=0.10 只有 CAGR 3.85%，f=0.25 有 9.63%（−5.78pp）。
        #  给数值则强制固定（如 0.10），换池子后需自己重扫。
        #  ⚠️ 峰值 f 会随池子/区间飘移（40 只池在 hs300/2019-2026 峰在 0.10，
        #     zz800/2020-2026 峰在 0.05）⇒ 自动模式只保证量级正确。
        #  资金规模无影响：10 万~50 万 CAGR 几乎不变（9.63→9.91），skip_lot=0，
        #     最低 5 元佣金与整手约束在这个范围不是瓶颈。
        "portfolio_f": None,
        #  并发持仓上限，0 = 不限（靠现金自然饱和，平台真实口径）。
        #  诊断：信号供给上限 均值 7.47 / 中位 6 / P90 17 ⇒ cap 超过 ~17 基本空转。
        "portfolio_cap": 0,

...
_DAILY_CACHE = {}     # ts_code -> {date_str: (open,high,low,close,pre_close,pct_chg,vol)}
...
def _preload_daily(code):
    """首次触及某代码时一次性拉全历史日线进内存（idx_daily_code 索引查询），
    之后 _raw_close/_raw_open/get_daily_row 全部走内存，消除逐笔 SQL。"""
    if code in _DAILY_CACHE:
        return
    conn = get_conn()
    df = pd.read_sql_query(
        "SELECT trade_date, open, high, low, close, pre_close, pct_chg, vol "
        "FROM daily WHERE ts_code=? ORDER BY trade_date", conn, params=(code,))
    conn.close()
    cache = {}
    for r in df.itertuples(index=False):
        cache[str(r.trade_date)] = (r.open, r.high, r.low, r.close,
                                    r.pre_close, r.pct_chg, r.vol)
    _DAILY_CACHE[code] = cache   # 无数据的代码也缓存空 dict，避免反复重查


def _raw_close(code, td):
    """不复权收盘价，精确匹配日期，无 fallback。"""
    _preload_daily(code)
    row = _DAILY_CACHE[code].get(td)
    if row is None or row[3] is None or pd.isna(row[3]):
        return None
    return float(row[3])
...
def qfq_close(code, td):
    """前复权收盘价 = raw × factor(td) / factor(latest)。
    adj_factor 随时间增大（hfq = raw × factor），故正确公式是 fac_t/fac_ref；
    反写会导致除权日把分红双重惩罚（2026-09-02 修复的根因）。"""
    ...
    if not f:
        return raw
    return raw * f / _ADJ_CACHE[code]["ref"]


def qfq_open(code, td):
    """前复权开盘价（与 qfq_close 同口径）。"""
...
def get_daily_row(code, td):
    ...
    if row is None:
        return None
    keys = ["open", "high", "low", "close", "pre_close", "pct_chg", "vol"]
    return {k: (float(v) if v is not None and not pd.isna(v) else None)
...
#  2. 财务因子（公告日口径，无前视）
# ════════════════════════════════════════════════════════════
_FI_CACHE  = None   # fina_indicator 全表缓存（ts_code, end_date, ann_date, debt_to_assets）
_BS_CACHE  = None   # balance_sheet 全表缓存（ts_code, end_date, ann_date, total_liab）
...
def _load_fi():
    ...
    if _FI_CACHE is None:
        conn = get_conn()
        _FI_CACHE = pd.read_sql_query(
            "SELECT ts_code, end_date, ann_date, debt_to_assets FROM fina_indicator "
            "WHERE ann_date IS NOT NULL AND ann_date != ''", conn)
        conn.close()
    return _FI_CACHE
...
def _load_bs():
    ...
    if _BS_CACHE is None:
        conn = get_conn()
        _BS_CACHE = pd.read_sql_query(
            "SELECT ts_code, end_date, ann_date, total_liab FROM balance_sheet "
            "WHERE ann_date IS NOT NULL AND ann_date != ''", conn)
        conn.close()
    return _BS_CACHE
...
def _liab_map(visible_date):
    ...
    缓存按 visible_date（每个调仓日独立缓存）。【内存版：一次性全表加载，逐周内存筛选，
    与原 SQL 版语义等价（ROW_NUMBER OVER ... ORDER BY end_date DESC, ann_date DESC 取1）】"""
    if visible_date in _LIAB_CACHE:
        return _LIAB_CACHE[visible_date]
    df = _load_bs()
    sub = df[df["ann_date"] <= visible_date]
    if len(sub) == 0:
        m = {}
    else:
        latest = (sub.sort_values(["end_date", "ann_date"], ascending=False)
                     .groupby("ts_code", sort=False).head(1))
...
def _debt_map(visible_date):
    ...
    【内存版：fina_indicator.ann_date 列声明 REAL（数值亲和），原 SQL 与 TEXT 参数
    比较时参数被转数值 → 数值比较，过滤有效、无前视。此处等价复刻。】"""
    if visible_date in _DEBT_CACHE:
        return _DEBT_CACHE[visible_date]
    df = _load_fi()
    v = float(visible_date)
    sub = df[df["ann_date"] <= v]
    if len(sub) == 0:
        _DEBT_CACHE[visible_date] = {}
        return _DEBT_CACHE[visible_date]
    latest = sub.sort_values(["end_date", "ann_date"]).groupby("ts_code", sort=False).tail(1)
...
def run_backtest(start_date="20210104", end_date="20260710",
                 ...
                 blacklist_days=BLACKLIST_DAYS, no_today_filter=False,
                 limit_filter="close"):
    if no_today_filter:
        limit_filter = "none"   # 兼容旧开关
    if limit_filter not in ("close", "open", "none"):
        raise ValueError("limit_filter 必须是 close/open/none")
    """主回测循环。"""
    ...
    def _do_sell(code, td, reason, price=None):
        ...
        del positions[code]
        # 黑名单冷却：blacklist_days<=0（rb0）时完全不冷却
        if blacklist_days > 0:
        ...
        if td in weekly and i > 0:
            ...
            if not new_codes:
                ...
                if cur != new_set:
                    ...
                    # 【2026-09-03 修复·资金部署率】先筛出真正可买的标的，再按「可买数量」
                    # 分配资金。原写法 cash/len(to_buy) 的分母含随后被跳过的标的，
                    # 跳过后预算不重新分配 → 现金大量闲置。实测 2022H1：
                    # 80 笔成交 / (80 成交 + 92 冷却 + 67 一字跌停) = 仅 33% 部署率。
                    buyable = []
                    for code in to_buy:
                        ...
                        if op is None:
                            continue
                        thr = _limit_thr(code)
                        if limit_filter == "close":
                            # 【前视】当日收盘 pct_chg + 全天 vol —— 实盘开盘时不可知
                            ...
                            if row["pct_chg"] is not None and row["pct_chg"] <= -thr + 0.005:
                                ...
                                continue
                        elif limit_filter == "open":
                            # 【实盘可得】开盘价即触及跌停 → 开盘无有效买盘，不参与
                            pc = row["pre_close"]
                            if pc and pc > 0 and op <= pc * (1 - thr) + 1e-6:
                                if verbose:
                                    print(f"  ⏸️ 跳过 {code}({_name(code)})：开盘跌停")
                                continue
                        buyable.append((code, op))
                    if buyable:
                        cash_per = cash / len(buyable)
                        for code, op in buyable:
    ...
    return _report(daily_vals, trades, trade_dates, start_date, end_date, zero_fee,
                   top_n=top_n, div_pct=div_pct, turn_pct=turn_pct,
                   debt_pct=debt_pct, size_pct=size_pct,
                   blacklist_days=blacklist_days, factor_lag=factor_lag)
...
def _report(daily_vals, trades, trade_dates, start_date, end_date, zero_fee,
            top_n=None, div_pct=None, turn_pct=None, debt_pct=None, size_pct=None,
            blacklist_days=BLACKLIST_DAYS, factor_lag=0):
    """【2026-09-02 修复】文件名改用实际运行参数（原用模块常量 TOP_N/DIV_PCT，
    导致 --top-n 5/15、--div-pct 10/40 的产物全部写进 n10_d25 文件名互相覆盖）。
    【2026-09-03】新增 rb/lag 后缀：--blacklist-days 0 → `_rb0`，
    --factor-lag N>0 → `_lagN`，复刻 7-14 旧版命名，默认值不产生后缀（行为中性）。"""
    top_n = TOP_N if top_n is None else top_n
    div_pct = DIV_PCT if div_pct is None else div_pct
    turn_pct = TURN_PCT if turn_pct is None else turn_pct
    debt_pct = DEBT_PCT if debt_pct is None else debt_pct
    size_pct = SIZE_PCT if size_pct is None else size_pct
    ...
    # 变体后缀（默认参数不加，保持旧文件名不变）
    rb_tag = f"_rb{blacklist_days}" if blacklist_days != BLACKLIST_DAYS else ""
    lag_tag = f"_lag{factor_lag}" if factor_lag else ""
    csv_path = (f"data/results/weekly_highdiv_vol/"
                f"backtest_n{top_n}_d{div_pct}_t{turn_pct}_db{debt_pct}_s{size_pct}"
                f"_{tag}{rb_tag}{lag_tag}_{start_date}_{end_date}.csv")
...
if __name__ == "__main__":
    ...
    p.add_argument("--blacklist-days", type=int, default=BLACKLIST_DAYS,
                   help=f"卖出后冷却交易日数，0=关闭黑名单（默认{BLACKLIST_DAYS}，文件名加 _rb0）")
    p.add_argument("--limit-filter", choices=["close", "open", "none"],
                   default="close",
                   help="跌停过滤口径：close=当日收盘pct_chg(前视，历史默认)；"
                        "open=开盘价触及跌停(实盘开盘即可知，非前视)；none=不过滤")
    p.add_argument("--no-today-filter", action="store_true",
                   help="等价于 --limit-filter none（前视隔离测试）")
    ...
    run_backtest(args.start_date, args.end_date,
                 ...
                 size_pct=args.size_pct, factor_lag=args.factor_lag,
                  ...
                  blacklist_days=args.blacklist_days,
                  no_today_filter=args.no_today_filter,

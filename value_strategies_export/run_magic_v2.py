...
在原版（run_dogs_annual.py --strategy magic，年初调仓）基础上加三件套：
  ...
  ④ 行业上限 + 固定持仓数
       —— 单一 stock_basic.industry ≤ 2 只；持仓数 = 全局 top_n（默认 5，不再写死 12）
...
#  ① + ④ 选股：EBIT 3年均值 + 行业上限
# ────────────────────────────────────────────────────────────
def select_magic_v2(prev_date, top_n=12, industry_cap=2, ebit_years=3,
    ...
    for code in sorted(eligible):
        ...
        recs.append((code, roc, ey, ind, avg_ebit, len(ebits)))
    ...
    if verbose:
        ...
        print(f"  [v2选股] 候选 {len(df)} 只 → 行业上限{industry_cap}/固定{top_n}只"
              f" → 实选 {len(out)} 只（{n1} 只财务不足{ebit_years}年）")
    return out
...
def run_backtest_v2(start_date, end_date, top_n=None, industry_cap=2,
...
def _report(daily_vals, trade_dates, capital, bench, trend_log,
    ...
    tag = "_breadth" if breadth_confirm else ""

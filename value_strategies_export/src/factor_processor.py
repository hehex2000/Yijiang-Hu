...
class FactorProcessor:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.factor_directions = {
            ...
            "TF4_turnover_pct": "neutral",         # 换手率自身历史分位：描述活跃度，不预测方向
            "TF5_bollinger_position": "negative",  # 接近下轨=低估买入区（0=下轨，1=上轨）
            "TF6_volume_price_regime": "neutral",  # 量价regime描述器(0常态/1放量价涨/2放量价滞/3放量价跌)
            "TF7_kdj_j": "neutral",           # KDJ 的 J 线：价格位置变化速度描述器（Jim第4期去魅），不进 technical_score、不预测方向
    ...
    def _calculate_scores(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        factor_groups = {
            ...
            # 注：TF4_turnover_pct(换手率分位) 与 TF6_volume_price_regime(量价regime)
            # 仅作描述器/风险 overlay，不预测方向，故不纳入 directional technical_score。
            "technical_score": ["TF1_ma_bullish", "TF2_MACD", "TF3_RSI", "TF5_bollinger_position"],

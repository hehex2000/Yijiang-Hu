...
class DogsOfMarketSelector:
    ...
    def select_stocks(self, date: str = None) -> pd.DataFrame:
        ...
        #
        # ⚠️ 2026-08-01 修复重大方向性 bug：
        #   原代码 dv_rank=rank(ascending=False) 让「高股息」拿到【最小】pct 值，
        #   pb_rank=rank(ascending=True) 让「低PB」也拿到【最小】pct 值，
        #   于是 score 越【小】越优；但第十步却按 sort_values(ascending=False) 取头部，
        #   等于每年稳定选出「存活候选里最不狗的 5 只」（反向选股）。
        #   实测 2014 hs300：应选交行(股息6.25%/PB0.70)等，实际选出大唐发电(2.36%/1.28)等，
        #   股息率差 3 倍、PB 差 1.6 倍 —— 此前所有狗股回测结论均受此影响。
        # 修正口径：rank 方向翻转，使 score 越【大】越优，与第十步降序取头部一致。
        df["dv_rank"] = df["dv_ttm"].rank(pct=True, ascending=True)    # 股息率越高 → rank 越大（越好）
        df["pb_rank"] = df["pb"].rank(pct=True, ascending=False)       # PB 越低   → rank 越大（越好）
        df["score"] = (df["dv_rank"] + df["pb_rank"]) / 2              # score 越大越好

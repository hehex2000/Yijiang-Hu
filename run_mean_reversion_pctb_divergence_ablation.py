# -*- coding: utf-8 -*-
"""
均值回归策略 —— %B 背离买入增强 A/B 归因
========================================

目的：验证 B站进阶视频(BV1FP876bEAo)提出的 %B 背离能否给现有均值回归策略带来
      真增量。控制单一变量，只开/关 pctb_divergence：

  C0  基线(无%B背离) : pctb_divergence=False  ← 当前默认
  C1  +%B背离买入     : pctb_divergence=True   ← 候选改进(opt-in)

%B 看涨背离逻辑（已在 mean_reversion_plugin 实现，opt-in）：
  价格创 K 日新低（窗口含 t-1 的 K 日极值）、但 %B 未同步创 K 日新低
  （动能衰减/抛压枯竭）= 看涨背离，作为均值回归的额外买入触发。
  全部用 t-1 及更早的滚动极值比较（均属过去，无未来函数）。

固定：股票池(沪深300 as-of 起始日快照前40，非幸存者偏差口径)、期间、每支本金、
      行情/手续费口径、其余参数全同 config。
注：吸取 headfake ablation 教训，build_universe 直接用 as_of_date=START 快照，
    不再用最新快照（避免幸存者偏差）。
"""
import sys
...
TOP_N = 40
BENCH = "000300.SH"
OUT_DIR = HERE / "data" / "results" / "mean_reversion_pctb_divergence_ablation"
...
def summarize(rows: list[dict], idx_ret: float) -> dict:
    ...
    return {
        ...
        "pctb_overlap_mean": float(np.mean([r.get("pctb_overlap", 0) for r in rows])),
    }


BASE = dict(config.STRATEGIES["mean_reversion"])
# 三对照：① 简单成本(基线口径) ② 大盘趋势门控 ③ 真实分科目成本
# 每组内部仅 pctb_divergence 单变量不同，隔离变量。
CONFIGS = [
    ("C0 基线",         dict(BASE, pctb_divergence=False, market_regime_gate=False, real_cost=False)),
    ("C1 +%B背离",      dict(BASE, pctb_divergence=True,  market_regime_gate=False, real_cost=False)),
    ("C0g regime门控",  dict(BASE, pctb_divergence=False, market_regime_gate=True,  real_cost=False)),
    ("C1g +%B+regime", dict(BASE, pctb_divergence=True,  market_regime_gate=True,  real_cost=False)),
    ("C0r 真实成本",    dict(BASE, pctb_divergence=False, market_regime_gate=False, real_cost=True)),
    ("C1r +%B+成本",    dict(BASE, pctb_divergence=True,  market_regime_gate=False, real_cost=True)),
]
...
def main():
    ...
    for name, cfg in CONFIGS:
        ...
        if cfg.get("real_cost"):
            tag.append("真实成本")
        print(f"\n### 运行 {name}  (pctb_divergence={cfg['pctb_divergence']}"
              f"{' · ' + '/'.join(tag) if tag else ' · 简单成本'})")
        rows = []
    ...
    # ── 归因表（全部 6 组）──
    print("\n" + "=" * 92)
    print(f"均值回归 %B 背离买入 A/B（{START}~{END}，沪深300前{TOP_N}，每支{CAPITAL//10000}万）")
    print("=" * 92)
    hdr = f"{'配置':<16}{'均值%':>9}{'中位%':>9}{'正收益':>8}{'跑赢指':>8}{'胜率%':>8}{'均交易':>8}{'均回撤%':>9}{'%B买入/只':>10}"
    print(hdr)
    print("-" * 92)
    ...
    def delta_block(a, b, title):
        ...
        print(f"  Δ%B买入 {cb['pctb_entries_mean'] - ca['pctb_entries_mean']:+.1f}/只  "
              f"(其中与 buy_signal 重叠 {cb['pctb_overlap_mean']:.1f}/只)")

    print("\n======== 三对照（各自隔离 pctb_divergence 单变量）========")
    delta_block("C0 基线", "C1 +%B背离", "① 简单成本口径（基线）")
    delta_block("C0g regime门控", "C1g +%B+regime", "② 大盘趋势门控（消除熊市接飞刀告警）")
    delta_block("C0r 真实成本", "C1r +%B+成本", "③ 真实分科目成本（佣金+滑点+过户费+日期感知印花税）")
    print(f"\n[基准 {BENCH}] {idx_ret:+.2f}%")
    ...
    print(f"\n[已保存] {OUT_DIR}/  (ablation_compare.csv + ablation_C0/C1/C0g/C1g/C0r/C1r.csv)")

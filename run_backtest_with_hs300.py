...
# ========== 命令行参数 ==========
parser = argparse.ArgumentParser(description='A股回测 - 多策略对比（含沪深300基准）')
parser.add_argument('--start-date', type=str, default='20230101', help='回测开始日期（格式:YYYYMMDD）')
parser.add_argument('--end-date', type=str, default='20231231', help='回测结束日期（格式:YYYYMMDD）')
parser.add_argument('--stocks', type=str, default=None, help='股票列表，格式: "code1,name1;code2,name2"（分号分隔）')
parser.add_argument('--total-capital', type=int, default=100000, help='每支股票初始资金（默认10万）')
parser.add_argument('--dca-amount', type=int, default=8000, help='月定投金额（默认8000元）')
...
DCA_AMOUNT_PER_MONTH = args.dca_amount
TURTLE_CHANNEL_PERIOD = args.turtle_period

# 双均线组合（使用命令行参数）
MA_COMBINATIONS = [
    (args.ma_short, args.ma_long, f'MA{args.ma_short}/{args.ma_long}'),
]
...
if args.stocks:
    STOCKS = []
    for stock_str in args.stocks.split(';'):
        code, name = stock_str.split(',')
...
def get_hs300_benchmark():
    ...
    try:
        ...
        logger.info(f"✓ 沪深300指数{START_DATE[:4]}年至{END_DATE[:4]}年收益率：{hs300_return:.2%}")
...
def run_backtest_for_stock(stock_code: str, stock_name: str):
    ...
    if df_turtle is not None:
        ...
        # 计算DCA策略的实际总投入和收益率
        dca_total_invested = strategy_dca.total_invested  # 使用策略内部追踪的总投入
...
def main():
    ...
    logger.info("="*60)
    logger.info(f"开始{START_DATE[:4]}年至{END_DATE[:4]}年回测（含沪深300基准对比）")
    logger.info("="*60)
    ...
    # 输出汇总报告
    print("\n" + "="*160)
    print(f"{START_DATE[:4]}年至{END_DATE[:4]}年回测结果汇总（含沪深300基准对比 + 股票本身涨跌）")
    print("="*160)
...
def print_order_details(result):
    ...
    for strategy_name, data in result['results'].items():
        ...
        print("\n" + "="*160)
        print(f"{stock_code} {stock_name} - {strategy_name} 交易明细")
        print("="*160)
        print(f"{'日期':<12} {'操作':<6} {'原因':<15} {'价格':>8} {'数量':>8} {'金额':>12} {'手续费':>10} {'印花税':>10} {'盈亏':>12}")
        print("-"*160)
        ...
        for order in orders:
            ...
            if action == 'buy':
                ...
                tax = 0
                profit = ''
                print(f"{date:<12} {'买入':<6} {reason_cn:<15} {price:>8.2f} {shares:>8} {amount:>12.2f} {fee:>10.2f} {tax:>10.2f} {profit:>12}")
            else:  # sell
                ...
                print(f"{date:<12} {'卖出':<6} {reason_cn:<15} {price:>8.2f} {shares:>8} {amount:>12.2f} {fee:>10.2f} {tax:>10.2f} {profit_str:>12}")
        ...
        stop_profit_count = sum(1 for o in orders if o.get('reason') == 'stop_profit')
        stop_loss_count = sum(1 for o in orders if o.get('reason') == 'stop_loss')
        end_sell_count = sum(1 for o in orders if o.get('reason') == 'end_of_backtest')
        
        print(f"买入笔数：{buy_count}，卖出笔数：{sell_count}")
        print(f"止盈卖出：{stop_profit_count}，止损卖出：{stop_loss_count}，到期清仓：{end_sell_count}")
        print()
    output_file = os.path.join(project_root, f'backtest_result_{START_DATE}_to_{END_DATE}_with_hs300_benchmark.txt')
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(f"{START_DATE[:4]}年至{END_DATE[:4]}年回测结果汇总（含沪深300基准对比）\n")
        f.write("="*140 + "\n\n")
        ...
        for result in all_results:
            ...
            if stock_code in stock_prices:
                ...
                f.write(f"  股票{START_DATE[:4]}年至{END_DATE[:4]}年涨跌：{stock_change:.2%}\n")
